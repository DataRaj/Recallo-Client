...
function normaliseConversation(raw: any): Conversation {
  ...
  if (raw.user1_name) {
    resolvedNames[user1Id] = String(raw.user1_name);
  }
  if (raw.user2_name) {
    resolvedNames[user2Id] = String(raw.user2_name);
  }
  // Some backends put the names nested: { user1: { id, name } }
  if (typeof raw.user1 === 'object' && raw.user1?.name) {
    resolvedNames[Number(raw.user1.id)] = String(raw.user1.name);
  }
  if (typeof raw.user2 === 'object' && raw.user2?.name) {
    resolvedNames[Number(raw.user2.id)] = String(raw.user2.name);
  }
