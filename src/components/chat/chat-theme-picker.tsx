...
import { CHAT_THEMES, useTheme, type ChatThemeKey } from '@/components/providers/theme-provider';
...
export function ChatThemePicker() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className="animate-slide-down absolute right-0 top-full z-50 mt-1 w-44 rounded-xl p-1.5 shadow-2xl"
          style={{ background: 'var(--color-chat-surface)', border: '1px solid var(--color-border)' }}
        >
          {(Object.keys(CHAT_THEMES) as ChatThemeKey[]).map(key => (
            <button
              key={key}
              onClick={() => { setChatTheme(key); setOpen(false); }}
